...
import { conceptStyles, renderConcept, renderIssue } from './feature-concepts.mjs';
...
for (const shot of manifest.screenshots) {
  ...
  if (shot.theme !== 'light') throw new Error(`The app has no ${shot.theme} theme: ${shot.id}`);
...
for (const theme of ['light', 'dark']) {
  // The app has one (light) theme, so both covers frame the same real capture;
  // only the canvas and the recreated panel follow the cover theme.
  const shot = shots.get(manifest.cover[theme]);
  if (!shot) throw new Error(`Missing ${theme} cover source`);
  outputs.push({ id: `cover-${theme}`, src: theme === 'light' ? 'readme-banner.png' : 'readme-banner-dark.png', width: 1600, height: 1000,
    ...
    html: html(`${manifest.cover.title} cover`, `<style>${conceptStyles}</style><div class="cover-brand"><img src="${icon}" alt=""><h1>${escape(manifest.cover.title)}</h1></div><div class="hero-copy"><h2>${escape(manifest.cover.headline)}</h2><p>${escape(manifest.cover.subtitle)}</p></div><div class="frame"><img src="${shot.uri}" alt="${escape(shot.alt)}"></div>${renderIssue()}<div class="hero-meta">Open source · Meta Ads + Google Ads</div>`, theme === 'dark' ? '#1c1719' : '#f7f1ee', `cover cover-ads ${theme}`),
  });
}
for (const feature of manifest.features) {
  ...
  const panels = renderConcept(feature.id, icon);
